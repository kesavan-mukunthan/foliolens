...
- **§6 — Benchmark-relative (gated on spec-benchmarks)** — excess return (fund minus benchmark, per period), beta, Jensen's alpha (CAPM regression intercept, monthly, vs TRI), Treynor, R², TE, IR, capture, taking a benchmark `Investment`. Alpha never ships as a naked point estimate: `MetricsResult` carries the t-stat and window alongside. *Accept:* own↔oracle ≤ 1e-6; benchmark passed as Investment; TRI variant only; alpha without t-stat fails the artifact schema.
