...
## Objective
Risk and risk-adjusted metrics over the monthly `ReturnSeries` built **eagerly, once, at construction** by the Investment factories (`share_class_from_nav` / `benchmark_from_index`, resampling month-end against the category `TradingCalendar`; persistence is spec-scale), validated **own-vs-oracle on frozen fixtures**. Metrics are pure free functions over `ReturnSeries`; `Investment` adapters read `.returns(freq)`; drawdown alone reads daily NAV. This layer assumes returns are **already present** (the panel is eagerly built and carried on the Investment), never recomputed from NAV inside a metric — except drawdown, by design.
...
- **§1 — Wire `.returns` + rf fixture Investment** — Implement `ShareClass.returns(freq)` and `Fund.returns(freq)` as a lookup into the eagerly-built `panels` map: the factories `share_class_from_nav` / `benchmark_from_index` compute both the monthly and daily `ReturnSeries` off the NAV once at construction (resampling month-end against the category `TradingCalendar`), and `Fund` prices via its representative `ShareClass`. Accept: `.returns(Frequency.MONTHLY)` returns a float64 ReturnSeries; NotImplementedError gone from both. Then wrap a frozen rf `ReturnSeries` as an `Investment` for test use (real sourcing is spec-benchmarks). *Accept:* rf series typed; consumed via the Investment contract; no live fetch.
