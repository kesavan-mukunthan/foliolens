...
- Month-end NAV = the NAV dated on the true last **trading day** of the calendar month, resolved from a cross-sectionally derived `TradingCalendar` (`returns/calendar.py`) — never assumed from the calendar days alone, and never a NAV dated after month-end (no look-ahead). A month is emitted only once its NAV series also carries at least one point in the strictly next calendar month (completeness: the still-accumulating trailing month is never emitted as if complete). The emitted point is relabelled onto the true calendar month-end date, not the trading day it was actually dated on — so every emitted date is a genuine month-end (`returns/monthly.month_end`).
- Daily NAV is the stored raw base; month-end is a derived series on top. Both are queryable. One `TradingCalendar` is derived per category class per run (e.g. the equity cohort) and shared across every fund and benchmark in it — never derived per fund, and never mixed across category classes (equity vs liquid/debt calendars are derived separately).
...
- **One data root, resolved one way.** `--data-dir` CLIs (`ingest.universe`, `report.flexipage.runner`) default to the `FOLIOLENS_DATA_DIR` env var when the flag is omitted; set it once per machine to the directory holding all three of `nav.parquet`, `index_nav.parquet`, and `scheme_master.parquet` together. The repo's own gitignored `data/` is scratch/test-fixture space only — never the root a real build points at. Two roots with the same directory name (`data/`) is exactly how a stale/orphan parquet gets loaded silently; if you ever have more than one candidate root, delete or clearly rename the loser rather than leaving both in place.
