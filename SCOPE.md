...
Working state between sessions lives in `docs/BOARD.md`.
...
- **FL-NL-1** *(origin: F4 commentary)* — **deterministic commentary fallback** for `null`-commentary funds: when the LLM call fails or refuses, render a metric-derived deterministic block rather than leaving the fund with no narrative. — Closed (PR #46; spec-flexicap-page §5b).
...
- **Return materialisation:** NAVs and reconciled trailing metrics as `decimal128`/Decimal. At fixture scale both the monthly and daily `ReturnSeries` panels are built **eagerly, once, at construction** by the factories (`share_class_from_nav` / `benchmark_from_index`) — resampling month-end against the shared per-category `TradingCalendar` — and are carried on the Investment's panel map, never persisted; daily returns remain a derived view over stored daily NAV. Panel persistence (completed months only, immutable float64 parquet) and the provenance stamp (per-scheme hash over month-end NAV inputs + code/convention version, full recompute on mismatch) are owned by `spec-scale`. The current in-progress month is derived on demand, never stored; daily NAV appends don't feed a completed month, so they don't trip the stamp.
