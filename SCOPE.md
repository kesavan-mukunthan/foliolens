...
- **Two ingestion mechanisms, deliberately distinct:** (1) *bulk backfill* — one-off per-scheme full-history fetch via mfapi.in (`ingest/universe.py`: resumable gzipped raw shards, polite sequential pacing — mfapi.in documents rate limiting despite "free" positioning; parallel fetching is counterproductive and prohibited); (2) *daily append* — the Cloud Run Job above, consuming AMFI's official `NAVAll.txt` (single file, all schemes, no intermediary). Backfill re-runs are exceptional (historical NAV corrections, quarterly at most); the daily append is the steady state and is what accumulates the survivorship-free record from its start date onward.
- **Panel validation (`validate_panel`, runs after every consolidation and daily append):** integrity gates — non-empty per scheme, no duplicate `(code, date)`, strictly increasing dates per scheme, NAV > 0, day-over-day moves >20% flagged for review (not auto-dropped), shard count reconciles to codes − failures, coverage histogram logged. Correctness gate — sample-based factsheet reconciliation (≈10 schemes across categories) via the existing oracle path, plus spot-check of latest NAVs against NAVAll.txt.
...
- Backfill validation cadence: is one-off factsheet reconciliation at consolidation sufficient, or should sampled reconciliation run on a schedule alongside the daily append's staleness alarm?
